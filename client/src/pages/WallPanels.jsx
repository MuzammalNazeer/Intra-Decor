...
const SUBCATEGORIES = ['All', 'Wood', 'PVC', 'Metal', 'Wainscoting', 'Upholstered'];
...
export default function WallPanels() {
  ...
  useEffect(() => {
    setLoading(true);
    let url = '/api/products?category=Wall Panelling';
    if (selectedSub !== 'All') {
  ...
  return (
    <div className="min-h-screen bg-[#f7f4f0] pb-20">
      ...
      {/* ── HERO BANNER ── */}
      <div className="bg-gradient-to-r from-[#4b2c2c] via-[#5c3030] to-[#251515] text-white py-16 px-4 text-center relative overflow-hidden">
        <div className="relative z-10 max-w-3xl mx-auto">
          ...
          <p className="text-sm text-gray-200 leading-relaxed max-w-xl mx-auto">
            Fluted natural oak acoustic slats, moisture-proof 3D PVC planks, velvet upholstered headboard blocks, and classic Georgian wainscoting kits.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-10">
        <div className="flex items-center justify-between mb-6">
          <span className="text-xs text-gray-500 font-medium">
            Showing <strong className="text-[#4b2c2c]">{products.length}</strong> architectural panels
          </span>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16 bg-white rounded-3xl border border-brand-border p-8">
            <p className="text-gray-500 text-sm mb-4">No wall panels found matching your selection.</p>
            <button
